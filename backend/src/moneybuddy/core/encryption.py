"""AES-256-GCM encryption for at-rest provider secret storage.
...
and provider-secret KEK. Rotating the JWT secret for token security reasons would silently
invalidate every stored secret (DecryptionError). Splitting via a dedicated
`encryption_key` decouples those two rotation policies.

Rollout: deploy with `MONEYBUDDY_ENCRYPTION_KEY` set on Vercel; existing v1
...
_V2_PREFIX = b"\x02"
_V2_HKDF_INFO = b"moneybuddy/provider-api-key/v2"
...
def _kek_material() -> bytes:
    ...
    if settings.jwt_secret_key:
        logger.warning(
            "MONEYBUDDY_ENCRYPTION_KEY is not set; falling back to jwt_secret_key "
            "as the provider-secret KEK. Set a dedicated encryption key to decouple JWT "
            "rotation from stored API keys."
        )
