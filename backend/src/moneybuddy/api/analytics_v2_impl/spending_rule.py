...
from moneybuddy.api.deps import CurrentUser, DatabaseSession
from moneybuddy.core.expense_class import is_capital_loss
from moneybuddy.core.query_helpers import as_naive, capital_loss_keys_for, inclusive_end
...
def _is_transfer_category(cat_lower: str) -> bool:
    ...
    the multi-part 'Transfer: <from> → <to>' pattern that moneybuddy's
    ...
    if cat_lower in _GENERIC_TRANSFER_LABELS:
        return True
    # "transfer:" (colon suffix) or "transfer: <anything>" -- the moneybuddy
...
def _prettify_savings_label(
    ...
    both the plain 'Transfer' category AND the moneybuddy default template's
