...
from moneybuddy import __version__
from moneybuddy.core.sync_engine import SyncEngine
from moneybuddy.db.session import SessionLocal, init_db
from moneybuddy.ingest.validator import ValidationError
from moneybuddy.utils.logging import logger, setup_logging

app = typer.Typer(
    name="moneybuddy",
    help="Production-ready financial data ingestion and reconciliation engine",
...
def version_callback(value: bool) -> None:
    ...
    if value:
        console.print(f"moneybuddy version {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """MoneyBuddy - Excel ingestion and reconciliation engine."""
...
def import_file(
    ...
    """Import a financial statement export and synchronize it with the database.

    The operation is idempotent: importing the same file twice results in no
    ledger changes.
    """
    # Setup logging
