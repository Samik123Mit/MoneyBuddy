"""add provider usage log table and user_preferences token-limit columns
...
def upgrade() -> None:
    # Provider usage log: one row per provider usage event.
    op.create_table(
        "ai_usage_log",
    ...
    # Per-user token limits (nullable = no limit)
    op.add_column(
        "user_preferences",
