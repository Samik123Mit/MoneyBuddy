...
Current for MoneyBuddy 2.24.0.

## Current Chain

- Script location: `src/moneybuddy/db/migrations`
- Revision files: 42
...
Alembic imports `moneybuddy.db.models`, which registers every model exported
from `moneybuddy.db._models`.
...
`moneybuddy.core.query_helpers` instead of raw `strftime`.
...
`main`. It uses the `MONEYBUDDY_DATABASE_URL` GitHub Actions secret. The
